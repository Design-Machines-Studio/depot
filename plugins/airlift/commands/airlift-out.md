---
name: airlift-out
description: Create or refresh a portable .airlift session handoff bundle.
argument-hint: "[--commit | --no-commit] [note]"
...
- `--no-commit`: default behavior. Capture the current uncommitted work as `.airlift/uncommitted.patch`; do not force a commit.
