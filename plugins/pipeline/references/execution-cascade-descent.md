...
**Step 3d.5 -- Rail-exhaustion ask gate.** RC 76 means every configured rail for this chunk is exhausted or gated; capacity is recoverable. If the interactive context can reach the operator, present the live rail status and offer exactly `wait` or `park`. Otherwise park resumably. The ask is scheduling only. It never selects a provider, authorizes another rail, broadens configured-key OpenRouter eligibility, weakens sensitive-path rules, or waives the final independent review.

`plugins/pipeline/references/rail-exhaustion-ask-gate.md` is the single owner of the complete gate behavior -- headless/`PIPELINE_EXHAUSTION_ASK=0`/non-operator/timeout parking, the `wait_category: human_gate` receipt with its named reset time and resume instruction, and the `exhaustionFallback` policy object. Do not restate that detail here.
