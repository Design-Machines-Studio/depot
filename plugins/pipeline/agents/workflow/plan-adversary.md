...
- **Event completeness:** "Where a current consumer, cross-module contract, realtime projection, or existing event contract creates an event obligation, does the required event publish AFTER commit? Never infer the obligation from SQLite mutation syntax alone."
