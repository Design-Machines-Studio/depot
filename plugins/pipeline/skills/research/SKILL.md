...
the canonical combined `payload-authorization.sh` snapshot. The default
`trusted-boundary` mode reruns the scanner and verifies unchanged bytes
immediately before contact without a per-payload prompt. With explicit
`OPENROUTER_PAYLOAD_AUTHORIZATION=exact-digest`, preparation returns
`PAYLOAD APPROVAL REQUIRED`. Per-file SHA lists or a child silently switching
