...
| `isolationStrategy` | enum | Optional receipt/ledger field, separate from `executionMode`. Closed set: `per-chunk-worktree` (default) or `sequential-on-branch` (orchestrator Step 1c detected a container-mounted test harness that verifies against the repo root). Recorded in the progress ledger, chunk receipts, receipt.md, and the Summary Report; the workflow-kernel adapters pass it through as the canonical `isolation_strategy` receipt field. It never widens the `executionMode` enum -- a receipt may carry both fields. |
