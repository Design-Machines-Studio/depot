...
"$WORKFLOW_KERNEL" emit-cost-summary --events plans/<feature-slug>/authoritative-receipts.json --output plans/<feature-slug>/run-cost-summary.json --receipt plans/<feature-slug>/receipt.md --matrix trusted-openrouter-bundle --repository-commit "$(git rev-parse HEAD)" $(test -n "$(git status --porcelain)" && echo --dirty-state) \
