...
test -L plans/<feature-slug>/receipt.md && { printf 'run receipt is a symlink; refusing to write it\n' >&2; exit 1; }
...
rm -f plans/<feature-slug>/run-cost-summary.json
"$WORKFLOW_KERNEL" run-cost-summary --events plans/<feature-slug>/authoritative-receipts.json --output plans/<feature-slug>/run-cost-summary.json --receipt-line plans/<feature-slug>/receipt.md --repository-commit "$(git rev-parse HEAD)" $(test -n "$(git status --porcelain)" && echo --dirty-state) \
  || { test -s plans/<feature-slug>/run-cost-summary.json && printf 'run-cost-summary: %s\n' plans/<feature-slug>/run-cost-summary.json >> plans/<feature-slug>/receipt.md \
       || printf 'run-cost-summary: skipped (%s)\n' "kernel-unavailable-or-failed" >> plans/<feature-slug>/receipt.md; }
```
