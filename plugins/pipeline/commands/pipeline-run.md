...
test -L plans/<feature>/receipt.md && { printf 'run receipt is a symlink; refusing to write it\n' >&2; exit 1; }
...
rm -f plans/<feature>/run-cost-summary.json
"$WORKFLOW_KERNEL" run-cost-summary --events plans/<feature>/authoritative-receipts.json --output plans/<feature>/run-cost-summary.json --receipt-line plans/<feature>/receipt.md --repository-commit "$(git rev-parse HEAD)" $(test -n "$(git status --porcelain)" && echo --dirty-state) \
  || { test -s plans/<feature>/run-cost-summary.json && printf 'run-cost-summary: %s\n' plans/<feature>/run-cost-summary.json >> plans/<feature>/receipt.md \
       || printf 'run-cost-summary: skipped (%s)\n' "kernel-unavailable-or-failed" >> plans/<feature>/receipt.md; }
```
