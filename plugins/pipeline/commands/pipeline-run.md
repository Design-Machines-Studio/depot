...
"$WORKFLOW_KERNEL" emit-cost-summary --events plans/<feature>/authoritative-receipts.json --output plans/<feature>/run-cost-summary.json --receipt plans/<feature>/receipt.md --matrix trusted-openrouter-bundle --repository-commit "$(git rev-parse HEAD)" $(test -n "$(git status --porcelain)" && echo --dirty-state) \
