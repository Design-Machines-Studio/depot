...
When every rail permitted for a chunk is exhausted or gated (cascade RC 76), the run pauses on a human gate instead of terminating BLOCKED. Capacity is recoverable; a terminal receipt is not. Before pausing the orchestrator fails closed in order: `PIPELINE_EXHAUSTION_ASK=0` restores the old hard block without pausing or asking; a missing `routing-policy.json`, a missing or malformed `exhaustionFallback`, or any `mode` other than `ask` does the same; sensitive-path chunks lose option (b) entirely; and a context that cannot reach the operator stops with `human_help_required` rather than fabricating the exchange. Only then does it emit an authoritative `progress` receipt with `wait_category: human_gate`, derive the offerable rails AT ASK TIME from the active host's `harness-profile.json` roles plus live `usage-probe.sh` headroom plus the interactive paths this session can reach, show the per-rail status (`capped until <time>` / `available this session` / `unknown`), and ask the operator to choose: (a) wait until reset, (b) authorize a fallback provider for THIS CHUNK in this run only, or (c) park. No offerable provider list is hardcoded anywhere in the ask path -- rail availability is per-operator and time-varying. Where a `usage-probe.sh` parser is a TODO stub the ask reports `unknown` rather than guessing. The operator is the human at the top-level interactive session; an agent, subagent, hook, auto-answer configuration, or automated harness is never an operator, and an ask answered by one is an unanswered ask.

Option (b) appends an authorization receipt -- `{"authorization_id": "<stable unique id>", "phase": "execute", "scope": "<run_id>", "chunks": ["<chunk-id>"], "provider": "<exact rail identifier the operator selected>", "authorized_by": "operator", "ask_evidence_ref": "<question id plus selected option>", "occurred_at": "<timezone-aware ISO-8601>"}` -- to the cumulative authoritative receipts BEFORE any fallback dispatch. A receipt without `ask_evidence_ref` is invalid and authorizes nothing. The `chunks` list may contain only chunk ids that have individually reached RC 76 at ask time, the authorization is consumed by one dispatch attempt per named chunk, and it expires the moment a permitted rail regains headroom -- including across a resume under the same run id. Every chunk executed under it leaves `requestedProvider` unchanged, records `attemptedProvider`, sets `implementedBy` to what actually produced the diff, sets `fallback: true`, and sets `fallbackReason: rail_exhausted_user_authorized`, at one authoritative write point per chunk. The requested provider is never relabelled to match the actual one; `implementedBy: claude` is legal under this authorization and only under it, because forcing a native Claude diff into a `codex` label would manufacture the dishonesty the boundary exists to prevent. A fallback dispatch without a prior valid authorization receipt MUST be halted and its output discarded, never merged. This is never a silent fallback: the authorization receipt is its only source of permission.

Ask-then-default-park is the only headless behavior: a non-interactive session, an ask that errors, an ask answered by a non-operator, or one that exceeds the caller's stated timeout parks resumable, and never assumes yes. `PIPELINE_EXHAUSTION_ASK=0` restores the old hard block for headless CI and is a kill switch on the ASK, never an enabler of silent fallback. Four exclusions survive every operator answer: the Workflow Authority broker gate is a security authorization boundary, not a capacity setting, so automated OpenRouter lanes stay fail-closed regardless of the answer (the interactive exact-digest `/openrouter` path may still be offered because it carries its own per-payload approval); the final full dm-review is never waived, the branch waits for capped review rails, and a review carrying a coverage gap on a required lane never satisfies that gate; no fallback-authorized review lane may share a provider family with the implementer of the code it reviews; and sensitive-path chunks are never implemented under fallback authorization. The routing policy object is `exhaustionFallback` in `plugins/pipeline/references/routing-policy.json`.
...
"$WORKFLOW_KERNEL" emit-cost-summary --events plans/<feature>/authoritative-receipts.json --output plans/<feature>/run-cost-summary.json --receipt plans/<feature>/receipt.md --repository-commit "$(git rev-parse HEAD)" $(test -n "$(git status --porcelain)" && echo --dirty-state) \
  || { s=$?; [ "$s" -eq 2 ] || [ "$s" -eq 6 ]; } || printf 'run-cost-summary: skipped (kernel-unresolvable)\n' >> plans/<feature>/receipt.md
```
