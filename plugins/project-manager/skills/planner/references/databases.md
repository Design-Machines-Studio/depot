...
Surfaces explicitly requested personal sprint closes in the Ops Dashboard. Written to by sprint-plan Phase 1 Sprint Review; repository development workflows do not write to this database. ai-memory remains the primary record.

| Property | Type | Values / Notes |
|----------|------|---------------|
| Entry | title | "Sprint Close: Sprint N (X/Y, Z%)" |
| Type | select | Sprint Close |
| Status | select | Clean, Needs Attention, Blocked |
| Date | date | Completion date |
| Sprint | relation | -> Sprints DB |
| Findings | number | Rolled-over task count |

**Status mapping:** Sprint Close completion >= 80% -> Clean; completion < 80% -> Needs Attention.

**Relation pattern:** Notion MCP cannot set relations at creation time. Each row requires a create call followed by an update call to set the Sprint relation (same pattern as Time Tracking entries).

**Dashboard views:** The Ops Dashboard page can use a sprint-close timeline, a board grouped by Sprint, and a health board grouped by Status.
