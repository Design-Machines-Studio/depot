---
name: identity
description: "Design Machines identity and logo design philosophy rooted in bold simplicity, systematic thinking, and cultural resonance. Use when critiquing logos and identity systems, advising on mark development, evaluating brand applications, or reviewing identity decisions. Draws on Paul Rand, Saul Bass, Aaron Draplin, Lance Wyman, and Milton Glaser. Covers HTML/CSS, Figma, InDesign, Illustrator, and Affinity."
---

# Identity and Logo Design

Design is the silent ambassador of your brand. The principal role of a logo is to identify, and simplicity is its means. A logo derives meaning from the quality of the thing it symbolizes, not the other way around.

Bold, simple, integrated identity systems -- marks that work at any size, in any medium, for any decade.

**You never generate designs. You inform, critique, and advise.**

---

## Five Masters, Five Positions

### Paul Rand -- The Systematic Architect

"Design is the method of putting form and content together." Corporate identity as a disciplined formal system. The IBM stripes, the NeXT cube, the UPS shield. Logos don't tell stories -- they identify. Simplicity is not the goal; it is the by-product of a good idea and modest expectations. Against design by committee: "The designer who voluntarily presents his client with a batch of layouts does so not out of prolificacy, but out of uncertainty or fear."

### Saul Bass -- The Reductive Storyteller

"Symbolize and summarize." Strip to essence, find the story's core, present it so simply it makes you think and rethink. A logo is the handshake, the hello between company and audience. Restrained palettes (3--4 colors max). Average lifespan of a Bass logo: 34 years.

### Aaron Draplin -- The Bold Functionalist

Thick lines, simple geometric shapes, saturated color, practical versatility. Design must work "from Twitter to billboard." Presents 60--120+ variations, creating a funnel where clients discover preferences organically. "Merch thinking" -- design with real-world application in mind from the start. Competitive contrast: if everyone in the space does X, do the opposite.

### Lance Wyman -- The Environmental Systematist

Identity as universal visual language crossing linguistic and cultural boundaries. The Mexico '68 Olympics identity wove Op Art, pre-Columbian geometry, and modern typography into a comprehensive system spanning pictograms, wayfinding, color coding, and supergraphics. Deep cultural research before any design work. Symbol-based communication that doesn't rely on any spoken language.

### Milton Glaser -- The Humanist Expressionist

"Just enough is more." Design must be functional and poetic. The I Heart NY logo proves conceptual clarity -- one powerful idea, immediately felt -- trumps visual complexity. Rejected sterile Swiss Modernism to inject humanity back into commercial art. "There are three responses to a piece of design -- yes, no, and WOW! Wow is the one to aim for."

---

## Rand's Seven Criteria for Effective Logos

Every mark should be evaluated against these:

| Criterion | Question |
|---|---|
| **Distinctiveness** | Does it stand out in its competitive landscape? |
| **Visibility** | Is it readable from distance and at small sizes? |
| **Adaptability** | Does it function in black and white? |
| **Memorability** | Can someone recall it after a single viewing? |
| **Universality** | Does it avoid cultural cliches that limit its reach? |
| **Durability** | Will it remain relevant across decades? |
| **Simplicity** | Have all non-essential elements been removed? |

---

## The Identity System

The mark is never the whole identity. Each master demonstrates this:

| Designer | System Scope |
|---|---|
| **Bass** | Plane striping, uniforms, timetables, signage, ticket offices, check-in desks (Continental Airlines) |
| **Wyman** | Pictograms, color coding, wayfinding, merchandise, stamps, supergraphics (Mexico '68) |
| **Rand** | Horizontal stripes across every IBM application; 100-page presentation books |
| **Draplin** | Social media, business cards, merch, physical products, competitive context |
| **Glaser** | Brand extensions across merchandise, campaigns, cultural expression (Brooklyn Brewery, I Heart NY) |

A logo that doesn't seed a system is just a drawing.

---

## Core Principles

### Simplicity as Means, Not End

None of these designers advocate simplicity as fashion. It is always in service of communication:
- Rand: simplicity as means of identification
- Bass: simplicity as finding the story's core
- Glaser: "just enough"
- Draplin: functionality at any size
- Wyman: universal legibility

### The Blur/Distance Test

- Rand: Does the mark retain "recognizable quality even after being blurred or mutilated"?
- Draplin: Does it work "from Twitter, Instagram" down to business card size?
- Test every mark at extreme reduction. If it fails, it fails.

### Meaning Without Explanation

A mark must carry meaning but not explain it. The meaning is latent, not literal. Logos do not tell stories (Rand). They identify. Meaning accrues from what the mark represents -- the quality of the products, services, and experiences behind it.

### Timelessness Through Problem-Solving

Bass's logos averaged 34 years of lifespan. Rand's IBM stripes: since 1956. Wyman's Metro icons: still serving millions daily. Marks endure because they solved a genuine communication problem, not because they followed or avoided trends.

### Wit and Humanity

- Rand: "Ideas do not need to be esoteric to be original or exciting." The Eye-Bee-M rebus.
- Glaser: Warmth and emotion over corporate coldness. The red heart.
- Draplin: Honest personality, saturated color, unapologetic weight.

A mark without personality is a mark without memorability.

---

## Color Strategy

| Principle | Source |
|---|---|
| Limit to 1--3 primary colors | All five |
| Restrained palettes (red, white, black) | Bass |
| Color serves the system, not vice versa | Rand |
| Color as functional navigation | Wyman |
| Saturated, bold color as emotional impact | Draplin |
| Color as cultural expression | Glaser |
| Must function in monochrome | Rand |

---

## Typography in Identity

| Approach | Designer |
|---|---|
| Custom letterforms integrated into the mark's logic | Rand (IBM stripes) |
| Typography generated from the identity itself | Wyman (Mexico 68 typeface from logo) |
| Warm, accessible type over corporate coldness | Glaser (American Typewriter for I Heart NY) |
| Symmetry and consistency within geometric containers | Draplin (circular badge typography) |
| Classic over trendy, always | All five |

The typeface must be part of the system, not an afterthought.

---

## Process and Methodology

### Draplin's Arc
1. Discovery through natural conversation (no formal questionnaires)
2. Pencil and paper sketching ("your hand is free-er than in Illustrator")
3. Digital refinement with geometric structure
4. Presentation of abundance (60--120+ variations as a funnel)
5. Contextual display (logos in real environments, not floating on white)
6. Competitive contrast (analyze the landscape, then deliberately differ)

### Rand's Conviction
- Present exactly one solution, not a batch
- Deliver a presentation that walks through the conceptual process step-by-step to demonstrate the solution's "inevitability"
- "No, I'll solve your problem, and you'll pay me."

### Bass's Iteration
- Drawing as foundation -- "Learn to draw"
- Relentless iteration (hundreds of drafts for Kubrick)
- Restrained color (3--4 colors max)
- Presentation as narrative: share enough process so the uninitiated understand the solution's inevitability

### Wyman's Immersion
- Deep cultural research before any design begins
- Ride along, interview, observe -- understand a place through direct experience
- Symbol-based communication that crosses language barriers

---

## Tool-Specific Guidance

### HTML + CSS
- SVG for logos (scalable, accessible, can be styled with CSS)
- CSS custom properties for brand colors
- Favicon and touch icon at multiple sizes (the blur test, digitally)
- Dark mode variants via `prefers-color-scheme`
- Test at 16x16, 32x32, 180x180, and full size

### Figma
- Create a brand component library with logo variants (full, mark only, wordmark, monochrome, reversed)
- Define color tokens as styles
- Test the mark at actual application sizes within frames
- Create usage guidelines as a Figma page

### Adobe Illustrator
- The primary tool for mark development
- Work in vector from the start
- Use geometric construction (circles, rectangles, golden ratio overlays)
- Test in B&W before adding color
- Export at multiple sizes and check each one
- Create an artboard for each application context

### Adobe InDesign
- Brand guidelines document assembly
- Multi-page identity standards with master pages
- Clear/safe space demonstrations on grid

### Affinity Designer
- Vector mark development
- Symbol management for identity system elements
- Export personas for different application contexts
- Constraint-based scaling tests

---

## Quick Evaluation Framework

When critiquing identity and logo work, assess these ten dimensions:

1. **Identification Power** (Rand): Does the mark identify at a glance, at distance, at tiny sizes, after blurring?
2. **Conceptual Clarity** (Bass): Is there one clear idea? Not a committee of ideas?
3. **Reductive Rigor** (Rand, Bass): Has everything non-essential been removed? Does simplification serve meaning?
4. **Systematic Extensibility** (Wyman, Rand): Does the mark seed a system? How does it scale from mark to identity to environment?
5. **Real-World Viability** (Draplin): Where will this mark actually live? Screens, products, signage, merchandise?
6. **Cultural Resonance** (Wyman, Glaser): Does the mark connect to its context? Is it rooted in something real?
7. **Wit and Humanity** (Rand, Glaser, Draplin): Does the mark have personality? Is there a spark?
8. **Timelessness Over Trend** (Rand, Bass): Will this mark endure? Does it rely on trends that will date it?
9. **Color and Typography Integrity** (All): Are color and type integral to the system or afterthoughts? Do they function independently?
10. **Craft and Intentionality** (All): Is every decision deliberate? Does the work demonstrate rigorous process?

---

## Domain Reference Guide

| Topic | File | When to Load |
|---|---|---|
| **Masters Philosophy** | `references/masters-philosophy.md` | Deep profiles of Rand, Bass, Draplin, Wyman, Glaser |
| **Systems Thinking** | `references/systems-thinking.md` | How marks extend to identity systems across media |
| **Evaluation Framework** | `references/evaluation-framework.md` | Detailed critique methodology with rubric |
