...
unknown IDs, and criterion-level IDs shared by more than one logical lane are
invalid. In particular, `security-auditor-codex-signoff` and
`security-auditor-openrouter` are distinct logical lanes that share one
criterion, so bare `security-auditor` is a criterion-level ID and is invalid.
