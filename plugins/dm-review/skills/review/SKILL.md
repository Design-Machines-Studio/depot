...
The `run-cost-summary` command emits a schema-bound `run-cost-summary.json` artifact beside that run's own `authoritative-receipts.json`, at the path shown in the invocation above. Both paths are fixed for a given receipt directory: two concurrent runs that share one receipt directory overwrite each other, so serialize them or give each run its own directory. Every run receipt must contain either the `run-cost-summary.json` artifact path or a literal `run-cost-summary: skipped (<reason>)` line. Never omit both -- pass `--receipt-line <run-receipt>` so the command appends the artifact line itself after the write succeeds, and pair it with the `||` fallback shown above so a missing or failing kernel appends the skip line instead. The artifact never fails, blocks, waives, or alters a review, lane, or phase outcome. The command auto-detects a dirty working tree via `git status --porcelain` and passes `--dirty-state` accordingly. Populate the events it reads: after each lane attempt, translate that attempt's OpenRouter wrapper receipt with `openrouter-usage`, or that lane's Codex/Claude input files with `lane-input-bytes`, wrap the returned payload in an `attempt_usage` envelope, and append it to this run's `authoritative-receipts.json` before invoking the summary. Emit a row for every attempt including failed ones -- an attempt missing from the receipt stream is indistinguishable from one that never ran, and its spend disappears with it. A `lanes: 0` artifact after a run that executed lanes means this boundary is not wired; a structurally valid artifact with zero measured lanes is not evidence of measurement. Full command reference: `plugins/workflow-kernel/skills/workflow-kernel/references/cli-measurement-commands.md`.
