...
**When this review is the pipeline's final full dm-review, option (c) and the headless gap-and-continue default are unavailable for coding-lane exhaustion.** The outcome is REVIEW INCOMPLETE and the branch waits, per the pipeline's non-overridable exclusion that the final full dm-review is never waived. A review carrying a coverage gap on a required lane never satisfies that gate; without this carve-out the review's continue-default would quietly cancel the pipeline's wait-default exactly when the review rails are exhausted.
