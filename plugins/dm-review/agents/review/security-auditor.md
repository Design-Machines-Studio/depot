...
`${CLAUDE_SKILL_DIR}/references/assembly-security-checks.md` and apply its
Assembly production checks -- NATS subject ACLs, SSE session validation,
object-level authorization, federation trust, authorizer pattern, dynamic SQL,
destructive confirmation, PII-safe logging, redirect validation, and the Auth
Boundary Map gap. That file also carries the cross-install trust choreography
and updater/release supply-chain checks, which apply only when the diff touches
`internal/federation/**` or `internal/updater/` and release tooling. For any
other project type, do not load it.
