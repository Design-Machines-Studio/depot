...
implementation_origin="$REPO_ROOT/plugins/dm-review/skills/review/references/implementation-origin.md"
...
require_text "$contract" 'Do not run `git worktree prune`' "contract forbids repository-wide worktree pruning"
...
require_text "$orchestrator" 'Do not sweep a namespace or run `git worktree prune`' "orchestrator forbids repository-wide worktree pruning"
...
for release_surface in "$assembly_release" "$assembly_release_skill"; do
  ...
  require_text "$release_surface" "Alpha staging fast path" "$release_rel defines the repository-driven alpha journey"
  require_text "$release_surface" 'direct `Publish Release Artifacts` dry run optional' "$release_rel skips the optional alpha artifact rehearsal"
  require_text "$release_surface" "authorization for every valid" "$release_rel treats the exact invocation as sole alpha authorization"
  require_text "$release_surface" "Continue through every valid alpha transition" "$release_rel continues the complete valid alpha journey"
  require_text "$release_surface" "do not request named acceptance" "$release_rel does not add post-verification acceptance"
  require_text "$release_surface" "Beta, RC, and stable retain" "$release_rel preserves stronger promotion boundaries"
  require_text "$release_surface" "Resolve the official staging target from current repository policy" "$release_rel resolves staging from Baseplate authority"
  require_text "$release_surface" "Do not infer that staging is a NED target" "$release_rel rejects NED inference"
  require_text "$release_surface" "Never hash RTK's compact" "$release_rel hashes raw HTTP bytes only"
  require_text "$release_surface" "beta and stable unchanged" "$release_rel prevents alpha movement of later channels"
  require_text "$release_surface" "An actual unsupported bridge still blocks." "$release_rel preserves support-floor bridge blocking"
  require_text "$release_surface" "missing authenticated-browser capture and a missing" "$release_rel reports non-blocking alpha coverage gaps"
  require_text "$release_surface" "CSRF, authorization, HMAC" "$release_rel preserves ordinary update UI protections"
  ...
  require_absent "$release_surface" '4. Dry-run `Publish Release Artifacts` directly.' "$release_rel removes the mandatory alpha rehearsal sequence"
done
...
require_text "$REPO_ROOT/plugins/pipeline/.claude-plugin/plugin.json" '"dm-review": ">=1.77.0"' "pipeline requires the provenance-aware dm-review release"
require_text "$REPO_ROOT/plugins/dm-review/.claude-plugin/plugin.json" '"model-router": ">=0.6.0"' "dm-review requires exact-diff origin routing"
require_text "$REPO_ROOT/plugins/pipeline/.claude-plugin/plugin.json" '"model-router": ">=0.6.0"' "pipeline requires exact-diff origin routing"
require_text "$implementation_origin" 'unknown provenance is one transparent nonblocking limitation' "ordinary unknown provenance is proportional"
require_text "$implementation_origin" 'unknown provenance keeps the required independent lanes incomplete' "sensitive unknown provenance stays incomplete"
require_text "$implementation_origin" 'rerun only the affected independent lanes' "provenance recovery reuses exact-head evidence"
require_text "$orchestrator" 'one cumulative implementation provenance set' "Pipeline full mode preserves all contributing receipts"
require_text "$pipeline_cmd" 'lean implementation still dispatches' "Pipeline lean mode preserves implementation provenance"
require_text "$codex_native_adapter" 'every successful live implementation and repair receipt ID' "Codex-native mode preserves implementation provenance"
require_text "$review_loop" "Preserve the initial review's complete implementation receipt IDs" "dm-review-loop preserves provenance across repair"
...
require_text "$assembly_build" 'Workflow Kernel `>=0.17.0`' "assembly resolves the exact-ref verification runtime floor"
