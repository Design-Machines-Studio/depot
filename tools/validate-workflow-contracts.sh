...
external_intake="$REPO_ROOT/plugins/dm-review/skills/review/references/external-finding-intake.md"
...
require_text "$review_skill" "Phase 1b: External Finding Intake and Evidence Fallback" "dm-review collects PR feedback before fallback"
require_text "$review_skill" 'For every PR-scoped full or quick review' "full and quick PR reviews require external intake"
require_text "$external_intake" 'inline comments/replies' "external intake includes inline comments"
require_text "$external_intake" 'submitted review' "external intake includes submitted reviews"
require_text "$external_intake" 'conversation comments' "external intake includes conversation comments"
require_text "$external_intake" 'check summaries/annotations' "external intake includes check annotations"
require_text "$external_intake" 'After repair/recheck and before reporting' "external intake refreshes once at settlement"
require_text "$review_loop" "Phase 1b settlement" "dm-review-loop preserves final external settlement"
require_text "$review_fix" "For external evidence" "dm-review-fix preserves external evidence"
require_text "$output_format" "**External findings:**" "review report accounts for external findings"
