...
require_text "$cascade" 'OPENROUTER_AUTHORIZATION_MODE=trusted-boundary' "pipeline wrapper rungs use configured-key automatic byte screening"
...
require_text "$agent_runner" "verify-trusted-boundary" "OpenRouter runner automatically screens unchanged payload bytes"
require_text "$pipeline_cmd" "Codex is the required adversarial reviewer" "Phase 5 requires the Codex adversarial lens"
require_text "$pipeline_cmd" "This planning phase stays native by policy" "Phase 5 remains native by workload policy"
