...
dm_review_dispatch="$REPO_ROOT/plugins/dm-review/skills/review/references/full-lane-dispatch.md"
dm_review_routing="$REPO_ROOT/plugins/dm-review/skills/review/references/openrouter-routing-reference.md"
dm_review_branch_a="$REPO_ROOT/plugins/dm-review/skills/review/references/openrouter-branch-a.md"
...
require_text "$cascade_descent" 'integration) PRIMARY_RAIL="codex"' "cascade descent fallback maps integration to Codex"
...
require_before "$orchestrator" 'execution-verification-profile.md' '### 3d: Dispatch Implementation Subagent' "contract evidence precedes implementation dispatch"
...
require_text "$dm_review_routing" '| `security-auditor-openrouter` | `moonshotai/kimi-k3` | `x-ai/grok-4.6` | 3600s |' "dm-review security analysis receives a one-hour completion budget"
require_text "$dm_review_routing" '| `pattern-recognition-specialist` | `deepseek/deepseek-v4-pro-0813` | `qwen/qwen3.8-max` | 1800s |' "dm-review routine analysis receives a 30-minute completion budget"
require_text "$dm_review_routing" '7200s at' "dm-review bulk analysis scales to a two-hour completion budget"
...
if [ "$(grep -Fc -- '--minimum-version 1.18.0' "$dm_review_dispatch")" -ge 1 ] &&
   ! grep -Fq -- '--minimum-version 1.14.2' "$dm_review_dispatch"; then
  printf "  OK    dm-review resolves only OpenRouter bundles compatible with the 1.18.0 disclosure contract\n"
else
  printf "  FAIL  dm-review resolves only OpenRouter bundles compatible with the 1.18.0 disclosure contract\n"
  failures=1
fi
...
require_text "$dm_review_dispatch" "OPENROUTER_AVAILABLE=true" "dm-review enables eligible configured-key lanes"
require_text "$dm_review_dispatch" "OPENROUTER_UNAVAILABLE_REASON=configured_key_or_bundle_unavailable" "dm-review records configured-key or bundle fallback"
require_text "$dm_review_dispatch" "OPENROUTER_SECURITY_POLICY_PATH" "dm-review resolves the installed OpenRouter security policy"
...
require_text "$dm_review_branch_a" 'a Claude `Agent` call is not a valid Branch A launcher' "dm-review keeps generic OpenRouter review off Claude execution"
