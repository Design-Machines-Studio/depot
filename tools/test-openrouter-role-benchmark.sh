...
assert jq -e '.qualityScore == 100 and .durationSeconds == 2 and .servedModel == "fixture/model-v1" and .usage.cost == 0.001 and .fallbackUsed == false' "$TMP/result"
...
while IFS= read -r case_id; do
  ...
  "$RUNNER" --score --case "$case_id" --output-file "$TMP/expected-output" \
    --receipt-file "$TMP/receipt" --result-file "$TMP/expected-result"
  assert jq -e --arg case_id "$case_id" \
    '.caseId == $case_id and .qualityScore == 100' "$TMP/expected-result"
