...
run_composition_checks() {
  ...
  printf "\n${BOLD}dm-review external finding intake:${RESET}\n"
  if ! "$SCRIPT_DIR/test-dm-review-external-finding-intake.sh"; then
    any_failed=1
  fi
  ...
  printf "\n${BOLD}Optional personal-source portability:${RESET}\n"
  if ! "$SCRIPT_DIR/validate-personal-source-portability.sh"; then
    any_failed=1
  fi
