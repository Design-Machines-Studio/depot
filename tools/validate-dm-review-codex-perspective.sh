...
security_path_requires_full() {
  case "$1" in
    internal/auth/*|*/internal/auth/*|internal/federation/*|*/internal/federation/*|*/security/*|*/middleware/auth*|*/middleware/security*|*/secretbox*|*/destructive_confirmation*|internal/baseplate/email/settings*|deploy/*|*/deploy/*|*.env*|*/.env*|*/openrouter-wrapper.sh|*/delegation-boundary.sh) return 0 ;;
    *) return 1 ;;
  esac
}
...
skill_authoring="$REPO_ROOT/docs/skill-authoring.md"
...
require_text "$review_skill" "model-router is the single" "review skill delegates concrete invocation to model-router"
reject_text "$review_skill" "codex exec -s read-only -c service_tier=fast" "review skill does not pin a concrete executor"
...
require_text "$consolidator" "Merge findings from every completed selected lane" "consolidator merges all selected-lane findings"
require_text "$consolidator" "a finding from any lane is in-scope" "consolidator treats every completed lane as actionable"
require_text "$registry" "codex-perspective" "agent registry includes codex-perspective"
require_text "$reviewer_output_contract" '### [P1|P2|P3] <title>' "canonical reviewer output contract normalizes output"
...
require_text "$output_format" 'participant=`reviewer-a`' "disputed example exposes anonymous participant"
...
require_text "$graceful_degradation" 'Optional enrichment unavailable' "degraded-lane behavior remains compatible"
...
reject_text "$output_format" '### Detailed Agent Reports' "report omits duplicated reviewer transcripts"
reject_text "$output_format" '[Full agent output verbatim]' "report does not require reviewer output verbatim"
require_text "$output_format" '### Raw Evidence Index' "report includes compact raw-evidence index"
require_text "$output_format" 'Raw output reference' "raw-evidence index includes raw output references"
require_text "$output_format" 'Raw output digest' "raw-evidence index includes raw output digests"
require_text "$output_format" '### P1 -- Critical (Blocks Merge)' "report retains canonical P1 detail"
require_text "$output_format" '### P2 -- Important (Should Fix)' "report retains canonical P2 detail"
require_text "$output_format" '### P3 -- Required Fix' "report retains canonical P3 detail"
require_text "$guardrails" 'Mark `Malformed`' "malformed lanes remain incomplete"
require_text "$guardrails" 'Mark `Partial` with fixed reason `empty response`' "empty required lanes do not become clean"
require_text "$reviewer_prompt_template" '## Required reviewer output' "reviewer prompts require findings-only output"
require_text "$reviewer_output_contract" 'Do not add recaps, praise, approvals' "reviewer prompts reject narrative padding"
require_text "$reviewer_output_contract" 'suppress a supported finding for brevity' "reviewer prompts preserve supported findings"
require_text "$skill_authoring" '## Bounded evidence for high-cost orchestration' "skill authoring defines high-cost evidence projection"
...
require_text "$REPO_ROOT/plugins/dm-review/.claude-plugin/plugin.json" '"version": "1.79.0"' "canonical dm-review version is 1.79.0"
require_text "$REPO_ROOT/plugins/dm-review/.codex-plugin/plugin.json" '"version": "1.79.0"' "generated dm-review version is 1.79.0"
require_text "$REPO_ROOT/plugins/pipeline/.claude-plugin/plugin.json" '"version": "1.66.0"' "canonical Pipeline version is 1.66.0"
require_text "$REPO_ROOT/plugins/pipeline/.codex-plugin/plugin.json" '"dm-review": ">=1.79.0"' "generated Pipeline dependency floor is current"
