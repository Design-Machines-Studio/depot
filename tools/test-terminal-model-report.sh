...
assert jq -e '.calls[0].requestedEffort == "high" and .calls[0].transmittedEffort == "high" and .calls[0].attempts[0].transmittedEffort == "high"' "$TMP/report.json"
assert jq -e '.calls[1].requestedEffort == "high" and .calls[1].transmittedEffort == "unavailable" and .calls[1].attempts[0].transmittedEffort == "unavailable"' "$TMP/report.json"
assert grep -Fq 'Effort requested / transmitted' "$TMP/report.md"
