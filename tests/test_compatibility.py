...
class CompatibilityTests(unittest.TestCase):
    ...
    def test_translation_works_from_installed_cache_layout_without_repo_ancestor(self):
        ...
        with tempfile.TemporaryDirectory() as directory:
            references = (
                ...
                / "workflow-kernel" / "0.5.0" / "skills" / "workflow-kernel"
                / "references"
            )
            shutil.copytree(
    ...
    def test_runtime_discovery_uses_the_shared_semver_compatibility_rule(self):
        ...
        self.assertEqual(compatible_kernel_version("0.5.0"), (0, 5, 0))
        self.assertEqual(compatible_kernel_version("0.5.10"), (0, 5, 10))
        for incompatible in (
            "1.0.0", "0.4.9", "0.3.9", "0.5", "0.5.0-rc1", 1, None,
        ):
        ...
        with tempfile.TemporaryDirectory() as directory:
            ...
            newer = install(root, ".claude", "0.5.1")
            install(root, ".claude", "0.5.0")
            self.assertEqual(
                resolve_workflow_kernel_runtime(pipeline, home=root / "home"),
                newer.resolve(),
            )
        with tempfile.TemporaryDirectory() as directory:
            ...
            pipeline = root / "depot" / "plugins" / "pipeline"
            pipeline.mkdir(parents=True)
            # Semver order, not lexical or mtime order: 0.5.10 > 0.5.9.
            newest = install(root, ".claude", "0.5.10")
            install(root, ".claude", "0.5.9")
            self.assertEqual(
                resolve_workflow_kernel_runtime(pipeline, home=root / "home"),
                newest.resolve(),
            )
        with tempfile.TemporaryDirectory() as directory:
            ...
            install(root, ".claude", "0.5.5", declared="0.5.4")
            with self.assertRaises(FileNotFoundError):
                resolve_workflow_kernel_runtime(pipeline, home=root / "home")
    ...
    def test_runtime_trust_accepts_both_supported_cache_roots_deterministically(self):
        ...
        for cache_name in (".claude", ".codex"):
            with self.subTest(cache_name=cache_name), tempfile.TemporaryDirectory() as directory:
                ...
                pipeline = root / "depot" / "plugins" / "pipeline"
                pipeline.mkdir(parents=True)
                plugin = root / "home" / cache_name / "plugins" / "cache" / "depot" / "workflow-kernel" / "0.5.0"
                package = plugin / "skills" / "workflow-kernel" / "references" / "workflow_kernel"
                package.mkdir(parents=True)
                (package / "__main__.py").write_text("")
                (plugin / ".claude-plugin").mkdir()
                (plugin / ".claude-plugin" / "plugin.json").write_text(json.dumps({"name": "workflow-kernel", "version": "0.5.0"}))
    ...
    def test_runtime_trust_serves_any_plugins_sibling_but_rejects_foreign_roots(self):
        ...
        with tempfile.TemporaryDirectory() as directory:
            root = Path(directory)
            plugin = root / "home" / ".claude" / "plugins" / "cache" / "depot" / "workflow-kernel" / "0.5.0"
            package = plugin / "skills" / "workflow-kernel" / "references" / "workflow_kernel"
            package.mkdir(parents=True)
            (package / "__main__.py").write_text("")
            (plugin / ".claude-plugin").mkdir()
