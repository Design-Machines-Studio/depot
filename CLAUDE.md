...
| **openrouter** | OpenRouter API provider plugin (leaf): big-diff analysis, second-opinion review, and one-shot text generation via GLM-5.2 (z-ai/glm-5.2, 1M context) and DeepSeek V4 over a single OpenAI-compatible endpoint. Powers the pipeline cascade's OpenRouter rail and dm-review's big-diff fallback. Privacy-pinned (OPENROUTER_ZDR). |
| **airlift** | Model- and harness-agnostic session-handoff capability. Writes a deterministic `.airlift/` bundle (HANDOFF.md, state.json, git-diff patch, RESUME_PROMPT.md) so a usage cap or rate limit becomes a non-event -- resume in any harness (Claude Code, Codex, DeepSeek, Gemini, Kiro, OpenCode). Tier-1 deterministic checkpoint (no model budget) plus optional ccusage early-warning monitor; wired into pipeline and dm-review phase boundaries. |
...
15. **External LLM provider failure without retry:** DeepSeek or Gemini runner fails and dm-review immediately classifies the agent as failed without attempting a Claude fallback. Core agent coverage gaps go undetected. Hardening (dm-review v1.30.0): Phase 4.5 External LLM Fallback in `plugins/dm-review/skills/review/SKILL.md` retries on Claude (sonnet) before applying failure policies; `plugins/dm-review/skills/review/references/guardrails.md` updated to trigger Phase 4.5 instead of immediate classification; `plugins/dm-review/skills/review/references/graceful-degradation.md` adds pre-classification fallback check.
