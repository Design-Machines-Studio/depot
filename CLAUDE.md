...
Depot targets Opus 4.8 (current Anthropic API flagship) and tunes through effort levers, not model-version branches. Agent `model:` fields use aliases (`opus`/`sonnet`/`haiku`) that auto-resolve to the latest release, so there are no literal model IDs to bump on a new flagship. Several agents pin an `effort:` frontmatter level to right-size cost: mechanical and external-LLM-wrapper agents run at `low`/`medium`, while `plan-adversary` runs at `xhigh` because it gates execution. See `docs/opus-4-8-tuning.md` for the effort matrix, the per-agent policy, and how to raise session effort (`/effort xhigh`, `ultracode`) for high-stakes runs. When editing agent frontmatter, confirm the level is supported by that agent's model (Haiku ignores effort; Sonnet 5 supports the full range including `xhigh`/`max`, but Sonnet 4.6 and earlier cap at `high`).

**Fable escalation (plan-conditional):** Claude Fable 5 (`fable`, Mythos-class, above Opus) is available only when the current plan carries it. Never pin `model: fable` in frontmatter. Instead: run the session on Fable so unpinned agents inherit it, and pass a dispatch-time `model: fable` override for the pinned decision gates (`plan-adversary`; the `execution-orchestrator` for the hardest runs) -- falling back silently to the frontmatter default if the dispatch errors as model-unavailable. Full rules in `docs/opus-4-8-tuning.md` (Fable Escalation section).

**GPT-5.6 family (Jul 2026):** OpenAI's Sol/Terra/Luna tiers replace GPT-5.5 on the OpenAI rails. `gpt-5.6-sol` is rank 98 and leads `frontier_api` on every host; `gpt-5.5` (96) stays one rung down as the fallback an older `codex-cli` can still reach -- reachable only because the orchestrator's **Native Model Descent** (RC 64) walks a native role's model list; `cascade-dispatch.sh` itself emits just the first floor-clearing model. Terra (91) is the mid rung. Luna (86) sits at the **frontier tail, never on `cheap_api`** -- at $1/$6 it is the cheapest OpenAI model but its output costs 3.4x GLM-5.2 and 6.9x DeepSeek V4 Pro, so `z-ai/glm-5.2` keeps the cheap head. Sol leads `native_judgment` on the **codex** host only; see `docs/opus-4-8-tuning.md` (Fable vs GPT-5.6 Sol) for why claude-code's native rail stays Claude-only. The Codex rail requires `codex-cli >= 0.144.x` (0.128.x rejects the 5.6 family), and `gpt-5.1-codex-mini` is unusable on a ChatGPT-sub account. Terra/Luna ranks are positional estimates from price and tier positioning, not benchmarks -- revise when SWE-bench numbers land.

**Kimi K3 (Jul 2026):** Moonshot AI's open-weight frontier model (`moonshotai/kimi-k3`, 2.8T-param MoE, native vision, 1M context, $3/$15 with $0.30 cache hits) joins the `frontier_api` rung at rank 91 (Terra tie) -- comparable to Opus 4.8/GPT-5.5 on Artificial Analysis (index 57), ahead of Opus 4.8 on GDPval-AA (1687 vs 1600), but discounted for Moonshot's own stated UX gap, ~20 TPS output, and max-only reasoning effort. It is the **only vision-capable model on the OpenRouter rail** (GLM-5.2 has none). Never on `cheap_api`/`bulk_api`/`openrouter_exec`: $15/M output is 8.5x GLM-5.2. **Privacy (demoted):** model selection priority is Quality > Price > Speed > Provider privacy; `OPENROUTER_ZDR=1` is opt-in only, so K3 is fully live by default even though its sole provider until the weights ship (planned 2026-07-27) is Moonshot's first-party API with no ZDR variant. Under opt-in ZDR it falls through as a per-model failure until a Western no-log host carries the weights. Rank is a positional estimate; revise when the technical report and SWE-bench numbers land.

**Subscription rails (Aug 2026):** the Claude plan downgrades Max -> Pro and Codex moves to Pro 20x, so the cascade is Codex-first. providerSplit is 60/15/25 (codex/claude/openrouter), `integration` chunks route to the codex class (only `ui` stays Claude-primary), the claude cascade class crosses to `premium_sub` (Codex Pro 20x) before paying `frontier_api` per-token rates, and `sonnet` leads the claude-code `native_judgment` ladder -- `opus`/`fable` stay as inert upgrade-path entries that RC-64 skips while off-plan. Treat Claude native as scarce, judgment-only spend reserved for ui/visual work.
