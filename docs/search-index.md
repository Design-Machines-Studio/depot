...
**45 skills** across 19 plugins | **38 agents** (22 review, 16 workflow) | **35 commands**
...
| airlift | airlift | session-handoff, checkpoint, usage-cap, rate-limit, resume, multi-model | -- | "session handoff", "usage cap", "resume in another model", "checkpoint before I run out", "hand off to Codex", ... |
...
| design-machines | audience | audience, positioning, pitching, competitive-landscape, federation, research | -- | "what's our positioning vs Loomio", "help me draft a pitch", "what do I say to the federation", "USFWC outreach", "CWCF outreach", ... |
...
| model-router | model-router | internal, model-routing, role-routing, dispatcher | -- | "dispatch a model role", "resolve a provider-neutral role", "route an independent reviewer" |
...
| ned | codify | compounding, lessons, codify, guardrails, continuous-improvement, postmortem | ai-memory | "codify", "compound", "capture the lessons", "what should we learn from this", "turn this into a rule", ... |
| ned | operate-ned | ned-operations, remote-development, ssh, account-selection, least-privilege, rootless-docker, tailscale | -- | "use NED", "run this on NED", "SSH to NED", "use the trav account", "use the ned account", ... |
...
| project-manager | assembly-coordinator | assembly, coordination, github, cross-repository, scope-guard, fixture-planning, role-routing, plan-comparison | -- | "fresh Assembly planning session", "Assembly Coordination Project", "next Assembly development chunk", "completed Assembly PR follow-up", "Fixture dependency planning", ... |
| project-manager | agent-message-board | agents, coordination, messages, source-links | -- | "agent message board", "shared agent handoff", "cross-project agent message", "reply to an agent handoff", "read source-linked agent messages" |
...
| workflow-kernel | workflow-kernel | workflow, state-machine, event-ledger, observation-index, recovery, resource-registry | -- | "validate a workflow-kernel run", "validate an existing Docker resource registry", "replay workflow events", "inspect workflow-kernel state", "diagnose a corrupt run ledger", ... |
...
| dm-review | migration-validator | review | migrations, goose, sql, validation, schema |
| dm-review | codex-perspective | review | codex, second-opinion, code-review, parallel |
