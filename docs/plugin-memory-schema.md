...
## Explicit Sprint Planning Dashboard Writes

The explicitly invoked `sprint-plan` workflow can write a structured sprint-close row to the **Agent Activity Log** database in Notion. Pipeline and dm-review development workflows do not write to Notion; their repository evidence, ai-memory observations, metrics, and receipts remain authoritative.

### Pattern

The sprint-close write is optional -- ai-memory is the primary record, Notion is a convenience view. It follows this pattern:
...
3. Create a page with sprint-close properties
4. Update the page to set the Sprint relation (two-call pattern)
