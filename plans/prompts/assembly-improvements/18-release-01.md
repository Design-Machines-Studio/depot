# RELEASE-01 — Repair false cross-branch release conflicts
...
```text
Implement only RELEASE-01: Repair false cross-branch release conflicts.
...
Later release-integrity repair. Reproduce the current failure before changing code; current main and active releases may already have corrected it.

Workspace:
Use /home/ned/ai/depot only to inspect Git metadata and fetch origin/main. Record the current exact SHA. If it has advanced from the prepared base, inspect the relevant delta and continue from refreshed main when the same scope remains valid; never revert newer work to the snapshot. Create a clean worktree at an unused path under /home/ned/ai/depot-worktrees and a new branch named docs/release-01 (use fix/ for executable changes). Choose a unique suffix if that branch/path already has an owner. Preserve the primary checkout and all existing worktrees: no stash, reset, clean, rebase, branch switch, removal or discarded files. Recheck open PRs and file ownership before writing; do not duplicate active work.
...
Task and non-goals:
Inspect tools/check-release-preflight.sh and its existing fixtures. Prior runs falsely treated the preserved merged #128 branch and a new unrelated source branch as independent dm-review 1.80.0 changes, even though the plugin tree was inherited byte-for-byte. Reproduce this exact case on current source.

Make the smallest comparator change that distinguishes a genuinely independent changed plugin tree from inherited/identical merged content. Preserve fail-closed checks for genuinely different same-version changes, missing evidence, version/tag integrity, dirty state and origin authentication. Preserve the installed-cache release check; this task does not authorize installations. Do not delete branches/worktrees, bump unrelated plugins, waive checks, rewrite the release system or add a ledger.

If the source is already corrected, verify the boundary cases and report no change. Keep release publication itself separate.

Acceptance and focused verification:
An unchanged inherited plugin and identical merged branch do not create a false independent-bump conflict; two genuinely different unmerged same-version plugin trees still fail. Missing or malformed comparison evidence stays closed. Run focused preflight fixtures and full composition for any actual code change. Do not tag or synchronize caches.

Consumer canary:
Run read-only preflight on the current real branch and classify each remaining failure. A cache mismatch remains a real release-coverage gap, not permission to mutate installations.
