# INSTRUCTIONS-06 — Audit Foreman instructions before runtime work
...
```text
Implement only INSTRUCTIONS-06: Audit Foreman instructions before runtime work.
...
Task and non-goals:
Inspect AGENTS.md, CLAUDE.md, nested instructions and repository-owned skills/hooks/agent configuration. Apply the shared concise, task-based development policy wherever a current conflict is demonstrated. Keep concrete execution provenance, trusted development assumptions, existing Pi ownership and Floor's observation boundary.

This is instruction/local-skill maintenance only. Do not modify src/pi-session.mjs, provider bindings, the fixed proof task, compaction, runtime UI, or dependencies. Record those runtime findings for HARNESS-01. If instructions are already aligned, produce a verified no-change result instead of an empty PR.

Acceptance and focused verification:
The local instructions do not hardcode every worker to a driver model or maximum effort, force blanket planning, or require personal systems. Existing runtime safety and task-monitoring authority remain intact. Check actual loaded scope and links; run focused hook tests only when relevant.

Consumer canary:
Show which instructions a bounded Pi runtime task would load and which verification authority it would use, without changing or claiming to test the adapter.
