# SKILLS-01 — Separate Assembly skill authority and reduce entrypoint load
...
```text
Implement only SKILLS-01: Separate Assembly skill authority and reduce entrypoint load.
...
Run after INSTRUCTIONS-01 is merged and the relevant consumer authority can be read at a current exact commit. Review the latest preceding result before starting; this is a reserved later prompt, not authorization to run all plugin passes together.

Workspace:
Use /home/ned/ai/depot only to inspect Git metadata and fetch origin/main. Record the current exact SHA. If it has advanced from the prepared base, inspect the relevant delta and continue from refreshed main when the same scope remains valid; never revert newer work to the snapshot. Create a clean worktree at an unused path under /home/ned/ai/depot-worktrees and a new branch named docs/skills-01 (use fix/ for executable changes). Choose a unique suffix if that branch/path already has an owner. Preserve the primary checkout and all existing worktrees: no stash, reset, clean, rebase, branch switch, removal or discarded files. Recheck open PRs and file ownership before writing; do not duplicate active work.
...
Task and non-goals:
Own plugins/assembly/skills/development/SKILL.md and its directly relevant references, manifest and fixtures. The audit found an approximately 46 KB entrypoint mixing prototype and production instructions. Inspect current Baseplate and prototype instruction authority read-only; do not design new product architecture.

Make prototype versus production task authority explicit and move detailed topic material into focused existing references where possible. Preserve current Go/Templ/Datastar patterns, trusted Fixture contracts, generation, upgrades, authorization, Live Wires reuse and exact prototype parity. Remove stale or conflicting directions based on source evidence, not model preference. No broad rewrite of unrelated Assembly skills, new abstraction layer, consumer code, or new framework.

Acceptance and focused verification:
Representative prototype, Baseplate and Fixture tasks load the correct authority without conflicting stack or product assumptions. Report entrypoint/reference bytes before and after as bytes, not measured tokens. Relevant trigger/contract fixtures and full composition pass; no necessary reference disappears.

Consumer canary:
Use current Baseplate and Fixture Jig instruction/development tasks as source-level canaries. Installed behavior requires a later approved tag/cache rollout; report that remaining proof separately.
