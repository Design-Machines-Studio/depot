# SKILLS-04 — Tighten strategy discovery and task-specific loading
...
```text
Implement only SKILLS-04: Tighten strategy discovery and task-specific loading.
...
Task and non-goals:
Own plugins/design-machines/skills/strategy/SKILL.md, directly affected references, discovery metadata and description evals. The audit found an oversized trigger description and broad loading. Narrow discovery to real strategy tasks and load detailed references when needed.

Preserve the company constraint: two trusted developers, small co-op installations, trusted internal Fixtures, products over bespoke enterprise infrastructure, Live Wires reuse, accessibility, maintenance and performance. Shared work must not require personal Notion/RAG/memory. Keep any optional personal context optional. No rewrite of company strategy, pricing, product roadmap, unrelated domain skills or consumer instructions.

Acceptance and focused verification:
Positive and negative trigger examples retain meaningful discovery, task-based loading preserves company constraints, and absence of personal systems does not block shared work. Report byte changes and eval results without claiming paid-token savings. Run relevant description/contract checks and full composition.

Consumer canary:
Use a Depot scope decision and a Fixture development proposal as instruction-level cases; verify both reject unsupported enterprise infrastructure while preserving real security requirements.
