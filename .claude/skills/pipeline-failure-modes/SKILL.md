...
description: Use when running, debugging, hardening, or reviewing a pipeline or dm-review run in this repo -- covers the 18 observed production failure modes with their root causes and hardening measures, the post-implementation verification checklist, and the postmortem index. Load before starting a pipeline run, when a run misbehaves, or when writing a postmortem.
---
